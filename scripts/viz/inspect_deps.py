...
def inspect_deps(handle: str, max_depth: int = 10):
    ...
    if not start_node_id:
        ...
        candidates = []
        for _node_id, data in store.graph.nodes(data=True):
    ...
    def print_tree(node_id, current_depth, visited_path, indent="", is_root=True):
        ...
        # Get outgoing edges. MultiDiGraph.get_edge_data returns {key: attrs}.
        children = []
        ...
        for i, (child_id, edge_type) in enumerate(children):
            ...
            print_tree(
                child_id, current_depth + 1, visited_path | {node_id}, next_indent, is_root=False
            )
