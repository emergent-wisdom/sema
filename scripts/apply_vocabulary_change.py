...
"""Apply staged vocabulary edits in the one order that works.
    ...
    apply_vocabulary_change.py --retarget-extends
...
REPO_DB = REPO_ROOT / "data" / "taxonomy.db"
...
def workflow_environment() -> dict[str, str]:
    """Pin subprocesses to this checkout's code and authoritative database."""
    env = os.environ.copy()
    env["SEMA_DB_PATH"] = str(REPO_DB)
    inherited_pythonpath = env.get("PYTHONPATH")
    env["PYTHONPATH"] = (
        os.pathsep.join((str(REPO_SRC), inherited_pythonpath))
...
def main() -> int:
    ...
    retarget_extends = ["--retarget-extends"] if args.retarget_extends else []
    ...
    step(
        "validate",
        [*sema, "apply", "--add", str(staging), "--check", *retarget_extends],
    )
    ...
    step(
        "apply to database",
        [*sema, "apply", "--add", str(staging), *retarget_extends],
    )
