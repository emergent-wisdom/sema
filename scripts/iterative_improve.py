...
HARDENING_TEMPLATES = {
    ...
    "signal": {
        "invariants": ["Signal/Noise ratio > 1.0"],
        "preconditions": ["Channel open"]
    },
    "consensus": {
        "invariants": ["Quorum > 50%"],
        "preconditions": ["Participants known"]
    },
    # New Templates
    "auction": {
    ...
    # Phase 2 Expansion
    "token": {
        "invariants": ["Token must be cryptographically signed", "Token has expiration or revocation mechanism"],
    ...
    "lock": {
        "invariants": ["Mutual exclusion: Only one holder at a time", "Deadlock freedom guaranteed (or timed out)"],
    ...
    "audit": {
        "invariants": ["Auditor is distinct from Actor (Separation of Duties)", "Audit trail is immutable"],
...
def auto_harden(store, node_id, handle):
    ...
    # Ensure lists exist
    if 'invariants' not in pattern: pattern['invariants'] = []
    if 'preconditions' not in pattern: pattern['preconditions'] = []
...
def run_improvement_loop(rounds=10):
    ...
    for nid, data in solutions:
        ...
        has_inv = False
        # Check graph edges for existing invariants
        for succ in store.graph.successors(nid):
            edge_type = store.graph.get_edge_data(nid, succ).get('edge_type')
            if edge_type == EdgeType.HAS_INVARIANT: has_inv = True
        
        # Force check for templates
