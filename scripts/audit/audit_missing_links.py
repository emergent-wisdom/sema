...
def audit_references():
    ...
    for handle, data in registry.items():
        ...
        # Check 'links'
        if 'links' in data:
            for rel, targets in data['links'].items():
                for t in targets:
    ...
    COMMON_WORDS = {
        "Task", "Vote", "Trace", "Signal", "Context", "Break", "Card", "Probe", "Rank", "Select", "Yield", "Compose", "Decay", "Rally", "Retry", "Route", "Warmup"
    }

    for handle, data in registry.items():
        # Aggregate all text
        text_content = f"{data.get('mechanism', '')} {data.get('gloss', '')} {data.get('failure_modes', '')}"
        for inv in data.get('invariants', []): text_content += " " + inv
        for pre in data.get('preconditions', []): text_content += " " + pre
        for post in data.get('postconditions', []): text_content += " " + post
