...
def audit_graph():
    ...
    print(f"Graph loaded with {store.graph.number_of_nodes()} nodes and {store.graph.number_of_edges()} edges.")
    
    problems = []
    
    # 1. Check for Orphaned Solutions (no edges at all)
    print("Checking for orphaned solutions...")
    solutions = store.get_nodes_by_type(NodeType.SOLUTION)
    for sol_id, data in solutions:
        degree = store.graph.degree(sol_id)
        if degree == 0:
            problems.append(f"[ORPHAN] Solution '{data['text']}' ({sol_id}) has 0 edges.")

    # 2. Check for Orphaned Components (not linked to any Solution)
    # Most components should be reachable from a Solution (Solution -> Component or Component -> Solution)
    # Actually, components like MECHANISM, OUTCOME are linked FROM Solution.
    # INVARIANT, PRECONDITION, POSTCONDITION are linked FROM Solution.
    # So incoming degree should be > 0.
    print("Checking for orphaned components...")
    component_types = [
        NodeType.MECHANISM, NodeType.OUTCOME, NodeType.PRINCIPLE, 
        NodeType.CRITICISM, NodeType.JUSTIFICATION, NodeType.NOVELTY,
        NodeType.INVARIANT, NodeType.PRECONDITION, NodeType.POSTCONDITION
    ]
    ...
    for c_type in component_types:
        ...
        for node_id, data in nodes:
            ...
            if in_degree == 0:
                 problems.append(f"[ORPHAN] {c_type.name} '{data['text'][:50]}...' ({node_id}) has no incoming edges.")
    ...
    for sol_id, data in solutions:
        ...
        if not category:
             problems.append(f"[MISSING_META] Solution '{data['text']}' missing 'category'.")
        if not layer:
             problems.append(f"[MISSING_META] Solution '{data['text']}' missing 'layer'.")
    ...
    for name, ids in name_counts.items():
        if len(ids) > 1:
            problems.append(f"[DUPLICATE] Solution '{name}' appears {len(ids)} times (IDs: {', '.join(ids)}).")
    ...
    for sol_id, data in solutions:
        ...
        if not (has_inv or has_pre or has_post):
             problems.append(f"[NAKED] Solution '{data['text']}' has no Invariants, Preconditions, or Postconditions.")
