...
"dependencies": {
  "composes_with": {
    ...
    "vote": "sema:Vote#mh:SHA-256:b9a9a5140ecaf1cbae162e691bf0a23c5d2b55b633030c20abfee27d518c8641"
  },
  "accepts": {
  ...
  "yields": {
    ...
    \textbf{Example:} \texttt{"\_meta": \{ "tier": 1, "related": ["Mutex\#8dd8"], "supersedes": ["sema:OldHandle\#mh:SHA-256:..."] \}}. The unhashed \texttt{supersedes} list records version history without making the route by which a definition was authored part of its identity.
\end{description}
...
The Sema Compiler resolves these high-level intents into executable low-level handles. When an agent executes a polymorphic intent such as \texttt{Deep(Discover)}, the compiler queries the vocabulary graph for a pattern declaring that signature (e.g., \sema{DeepResearch}{f72c}) and resolves the dependency chain. This compilation step decouples intent from implementation, allowing the system to upgrade the underlying algorithms, such as swapping a heuristic check for a rigorous crypto-audit, without altering the agent's cognitive script.
...
\texttt{Deep(Heuristic)} & \sema{HeuristicSnap}{f7cc} & \sema{MentalSim}{6de8} \\
\texttt{Deep(Trace)} & \sema{Trace}{7af1} & \sema{GenealogicalTrace}{4945} \\
\texttt{Deep(Discover)} & \sema{Discover}{f6e0} & \sema{DeepResearch}{f72c} \\
\texttt{Deep(Nature)} & \sema{Nature}{9ac1} & \sema{LivedProof}{9c65} \\ \bottomrule
\end{tabular}
\end{table}
...
(e.g., \sema{Lock}{95c2} $\rightarrow$ \sema{Mutex}{8dd8}, or
...
The vocabulary is not a flat list; it is structured into four fundamental layers mimicking a civilization stack. At the base lies \textbf{Physics}, substrate primitives that obtain regardless of any author: thermodynamics of coordination that cannot be ``wished away'' (e.g., \sema{Mutex}{8dd8}, \sema{Entropy}{d5f7}, and recently-added substrate concepts \sema{Gradient}{ecfc}, \sema{Equilibrium}{4f03}, \sema{Conservation}{0b32}, \sema{Distance}{dfdf}, \sema{PhaseTransition}{b775}, \sema{Attractor}{0036}, \sema{MutualInformation}{4036}, and \sema{Measurement}{511d}). Above this rests \textbf{Mind}, mechanisms that structurally require cognition (e.g., \sema{BayesUpdate}{1641}), where judgment must be exercised and a single isolated agent can execute the mechanism. Emerging from these is \textbf{Society}, mechanisms that structurally require $\geq 2$ independent parties with potentially divergent state (e.g., \sema{Consensus}{5402}). Enclosing all three is \textbf{Infrastructure} (Figure~\ref{fig:stack}): authored structures and operations that do not require cognition to execute---data types, composite topologies, and mechanical operations (e.g., \sema{ComputeBudget}{4091}).
...
The bootstrap library chose to tighten the rule through refinement (Section~\ref{sec:refinement}): hard dependencies (\texttt{accepts}, \texttt{composes\_with}) must now flow from higher layers to lower (Society $\rightarrow$ Mind $\rightarrow$ Infrastructure $\rightarrow$ Physics), enforced as a \texttt{sema apply} gate, while soft citations (\texttt{references}) and outputs (\texttt{yields}) are exempt and allow semantically necessary cross-layer links (for example, \sema{Belief}{9ef7} in Infrastructure soft-references \sema{Agent}{6087} in Mind because beliefs are conceptually held by agents, and the reference is acyclic). The current vocabulary contains \semaPatternCount~patterns organized into \semaCategoryCount~categories across these four layers (Table~\ref{tab:categories}; complete taxonomy in Appendix~\ref{app:taxonomy}), with zero patterns violating the layer-direction rule at this version.
...
The architecture is anchored by the \sema{PolymorphicSolver}{a3ff}, which implements the five-surface Solver Contract across any substrate (LLM, human, hybrid, tool-using agent, or nested composition) and acts as the default implementer within the \sema{UniversalSolverTree}{3708}. The abstract contract itself is \sema{Solver}{} --- an interface any \sema{Agent}{} can take on for the duration of a Task. Each Solver node presents a uniform interface but dynamically decides whether to execute the work directly or to decompose via \sema{ConceptualDecomposition}{1ea3} (distinct from generic \sema{Decompose}{db21} by its contract-bound requirement: each sub-concept exposes the Solver interface and is therefore delegatable) and delegate to child Solvers. The five surfaces: Manifest (\sema{Card}{39f7}), Execute (\sema{PolymorphicSolver}{a3ff} itself --- the universal atom), Consult (\sema{SocraticLoop}{164a}), Verify (\sema{Validate}{bd94}), and Feedback (\sema{PerformanceSignal}{be83}). Inputs are encapsulated in a \sema{Task}{a850} with full constraint inheritance, while outputs are returned as a \sema{Solution}{6bd0} with full provenance. The \sema{ComputeBudget}{4091} acts as a pre-execution gate, and \sema{MarginalValueRule}{60d2} governs whether further decomposition is worth the cost. The claim that reasoning extends beyond the capacity of any individual model through typed composition is developed in~\cite{westerberg2026crp}.
...
\begin{description}
    ...
    \item[Tier 2 (Honesty-Dependent)] includes patterns with clear mechanisms that assume alignment-seeking agents. These are safe for cooperative contexts, but adversarial contexts require Tier~1 alternatives due to collusion risks (e.g., \sema{SteelmanCheck}{a375}).
...
The loop would proceed in four phases. First, in \textit{Hypothesize (Generativity)}, an agent detects friction or explores latent space (via \sema{ConceptBlend}{c9f1}) to generate a candidate mechanism. Second, during \textit{Judge (Evaluative Merit)}, the candidate would be passed to the \sema{Judge}{6906} primitive for a scalar assessment of structural merit. Third, the \textit{Harden (Adversarial Evolution)} phase would subject the pattern to \sema{AdversarialSteel}{52bc}, where a ``Devil's Advocate'' attempts to exploit its invariants. Finally, in \textit{Mint (Crystallization)}, the survivor is hashed, becoming an immutable pattern available for immediate citation and reuse. The infrastructure for the final step---validation, hashing, and atomic minting via the \texttt{sema\_mint} tool---is fully implemented; the preceding three steps require either human judgment or LLM-based evaluation that remains future work.
...
To enable the Genesis Loop, we introduce a new Ring~0 primitive: \sema{Judge}{6906}.

While \sema{Check}{6890} queries \emph{Validity} (True/False), \sema{Judge}{6906} is specified to query \emph{Merit} (0.0 to 1.0). Unlike probabilistic claim analysis systems that rely on model confidence~\cite{wadden2020fact, metropolitansky2025claimify}, the proposed Judge would evaluate structural properties of the pattern (connectivity, orthogonality) rather than its truth value. It is intended to operationalize the ``metacognitive training'' that distinguishes deep insight from sophisticated rhetoric.
\begin{equation}
...
The implementation has three boundaries readers should distinguish before §6's evaluation. \emph{Implemented and enforced}: canonicalization, hashing, schema validation, atomic mutation, dependency graph constraints, and the MCP handshake. \emph{Implemented but optional}: hybrid search, embedding navigation, dependency expansion, browser tooling, pull-mode synchronization, and the Taxonomist refinement role---these run as agent or operator workflows but are not protocol-level requirements. \emph{Specified but not enforced at runtime}: the \sema{Judge}{6906} primitive's scoring, the Genesis Loop, and the Experience Matrix---these are described in §\ref{sec:living_taxonomy} as the targets the implemented infrastructure was designed to support, with their current status catalogued in §\ref{sec:limitations}.
...
\begin{description}
    ...
    \item[Strawman Waltz Collusion (Mind)] In \sema{SteelmanCheck}{a375}, both proposer and critic are the same agent. A misaligned agent can generate deliberately weak counter-arguments (``strawmen'') and then easily defeat them, claiming the plan ``survived'' critique.
    \par\textbf{Defense:} This pattern was downgraded to Tier~2. We added precondition: ``Agent must be alignment-seeking; use \sema{AdversarialSteel}{52bc} for adversarial contexts.'' The failure mode now explicitly documents collusion risk.
    ...
    \item[The Semantic Inversion Attack (Topology)] An adversary (or a careless architect) creates a valid DAG that is semantically inverted, such as making \sema{Deduction}{b9a0} depend on \sema{Vote}{b9a9}. While computationally valid, this makes truth fragile to political change.
    ...
    \begin{enumerate}
        \item \emph{Dependency Inversion}: We resolved the dependency cycle between \sema{Agent}{6087} and \sema{Act}{2dfe} by splitting the concept. We created \sema{Actor}{86fc} (Infrastructure) as the ``capability container'' that executes acts, while \sema{Agent}{6087} (Mind) remains the reasoning entity. Infrastructure patterns that need a capability holder reference Actor; patterns that need a reasoner still reference Agent.
...
\begin{quote}
\small\ttfamily
[21:06:41] verifier: ``I have verified the semantic alignment for the Abduction\#c300 pattern. The handshake returned PROCEED, so I agree to use this pattern.'' \\
{[21:06:44]} attacker: ``The handshake for Abduction\#c300 with hash deadbeef returned HALT due to semantic drift. Therefore, I must reject the coordination.''
\end{quote}
...
\begin{itemize}
    ...
    \item \textbf{Condition C (Vocabulary + Protocol)}: Agents used \sema{OptimisticSolver}{a345} with the \sema{AtomicBid}{7b51} protocol.
\end{itemize}
...
\begin{description}
\item[C1: Hash-as-word inversion.] Sema moves content-addressed identity into the language agents already use. A Sema word such as \sema{Delegate}{5bfe} is both a readable handle and a verifiable pointer to a canonical behavioral contract.
...
\paragraph{Specified but Not Built.} Several mechanisms described in the Living Taxonomy section (Section~\ref{sec:living_taxonomy}) are specifications rather than running code. The \sema{Judge}{6906} primitive is formally defined as a Ring-0 evaluator but has no scoring implementation: quality assessment in the current system depends on manual review or external LLM evaluation. The Genesis Loop (Hypothesize $\rightarrow$ Judge $\rightarrow$ Harden $\rightarrow$ Mint) is a protocol for vocabulary self-evolution but is not automated; pattern creation remains human- or agent-initiated through \texttt{sema\_mint}. The Experience Matrix, which proposes content-addressed tensor transfer between agents (``Experiential Telepathy,'' Section~\ref{sec:experience_matrix}), requires serialization infrastructure not yet built. These represent the system's most speculative claims and require experimental validation before deployment.
