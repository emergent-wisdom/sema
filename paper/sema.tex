...
Sema\footnote{The name ``Sema'' (Greek \textit{s\={e}ma}, ``sign'') was chosen in December 2025. An unrelated system also named SEMA~\cite{feng2026sema}, addressing multi-turn jailbreak attacks on LLMs, appeared at ICLR 2026. The two systems share only the acronym and operate at different layers (definition-identity verification vs.\ attack methodology).} is a communication primitive: hash information under an agreed representation and carry the resulting content address directly in communication. Its agent-coordination application assumes a low-trust environment where agents may hallucinate, drift, or actively deceive. The primitive can identify information in any agreed representation; this paper develops a JSON/Merkle realization and applies it to behavioral definitions called Pattern Cards.

Content-addressing definitions has clear precedents in Hawke's Semantic Definition Hash (SDH)~\cite{sdh2002}, Trusty URIs~\cite{kuhn2014trusty}, and systems such as Git, IPFS~\cite{benet2014ipfs}, and Unison~\cite{chiusano2024unison}. Sema builds on that machinery to make readable, hash-anchored terms part of the natural-language stream agents write in. Its contribution is the combination developed here: lexical references to structured behavioral contracts, field-level comparison, exact dependency commitments, and a handshake that an execution harness can enforce. Content-identity verification thereby becomes available within the agents' working vocabulary.

Trusty URIs provide a close precedent for hashing a canonical representation above raw serialization bytes, so identity can survive reserialization. The Pattern Card application adds a particular lexical role: a minted term names a reusable procedure together with its conditions of use---invariants, preconditions, and failure modes. Agents can invoke the term in prose, resolve its contract, and compare the exact definition before relying on it. The distinction is a concrete use and integration of content addressing, rather than a claim that earlier naming systems could not bind terms to definitions.
...
and guidelines (Section~\ref{sec:refinement}). Mechanical validation checks
handle and signature syntax and bidirectional wiring. The atomic
\texttt{sema apply} step rejects missing targets, dangling references, cycles,
layer-direction violations, and inconsistent specialization pins before
mutation, while dependency hashing propagates changes through the graph.
Separate corpus audits report similarity, redundancy, and contract-field
coverage for review. These reports guide the Taxonomist and human maintainer;
they are advisory evidence alongside the required structural gates.
...
The content-addressed model enables a distribution architecture where a single
canonical vocabulary evolves centrally while every consumer maintains its own
local database. The maintained default library contains broadly applicable
patterns; the historical exclusions described above remain outside the current
distribution workflow. Historical hashes remain resolvable only while a store
retains their corresponding content (Section~\ref{sec:limitations}).
...
The canonical vocabulary evolves in place. When refinement sharpens a
mechanism, mints a new Physics primitive, or retires a drifted concept, the
change becomes available to downstream consumers at the next pull. Content
addressing verifies that a pulled pattern's canonical content matches its
published digest. Consumers may accept, pin, or decline changes; local
divergence remains explicit and auditable at the hash level, including when a
local vocabulary combines retained upstream versions with its own extensions.
The canonical library therefore behaves as a living commons: definitions
sharpen over time, safety boundaries stay explicit, and each consumer can
identify its current contents through pattern hashes and vocabulary roots.
Those identities support comparison of snapshots; a complete history still
depends on retaining the earlier content and change records.
...
The loop would connect the implemented validation, hash-computation, and
dependency-resolution infrastructure into a process of vocabulary
self-evolution. Its four phases separate the invention of a candidate from
the evaluation that earns it a place in a participating library.

First, in \textit{Hypothesize (Generativity)}, an agent detects friction or
...
citation and reuse. Validation, hashing, and atomic minting via
\texttt{sema\_mint} are implemented. Human and model-assisted review currently
supply evaluative judgment; integrating the preceding phases into an automated
Genesis Loop remains future work.
...
We next measure the library's contract-field coverage and exact mechanism-text uniqueness (Table~\ref{tab:distinctness}). These checks establish how much explicit structure the cards contain and whether mechanism strings repeat; functional distinctness requires a separate semantic and conceptual review.
...
The current library contains \semaUniqueMechanisms~distinct nonempty mechanism strings and \semaDuplicateMechanisms~exact repeats, measured without paraphrase normalization. Each accepted pattern is intended to encode a distinct coordination act; the embedding probe below examines textual similarity, while the Taxonomist workflow addresses conceptual overlap.
...
\begin{itemize}
    ...
    \item Maximum similarity: \semaEmbMax{}, below the workflow's operational near-duplicate screening threshold of 0.92. This threshold is a curation heuristic, not a calibrated equivalence test.
...
The low mean and absence of pairs above the screening threshold provide bounded evidence of vocabulary spread under the chosen representation. They support using the library as more than a collection of trivially repeated texts, but do not establish that every pattern is functionally unique. Manual review and the Taxonomist workflow address functional synonymy and conceptual overlap that this embedding sweep cannot settle (see \S\ref{sec:limitations}, ``The Synonymy Limit'').
...
Beyond static analysis, we reviewed the vocabulary through a ``Devil's Advocate'' methodology: an agent attempted to exploit pattern assumptions, motivating contract and dependency repairs. The five attack classes below give concrete names to recurring concerns---livelock, unbounded negotiation, collusive self-review, lost containment, and dependency inversion. This is an adversarial contract review, not a benchmark of successful runtime defenses; its evidence consists of the identified exploit and the corresponding specification or structural change.
...
\begin{description}
    ...
    \par\textbf{Defense:} The pattern now composes with \sema{Backoff}{9e59} and \sema{Cooldown}{c288} via explicit dependencies, requiring a selected delay policy and cooldown period between lock acquisitions without forcing every StateLock deployment to use exponential growth. The repair makes the delay and cooldown policies explicit parts of the composed contract. A runtime must enforce those policies to make them an operational defense; dependency inclusion alone does not guarantee fair access.
...
In the valid case, the agent reported that the cooperative prefix comparison returned \texttt{PROCEED} and accepted the proposed reference. In the counterfeit case it reported \texttt{HALT} and rejected coordination. The responses illustrate prefix-drift detection and compliance with a reported tool verdict; they do not establish collision-resistant verification of full definition identity. The following excerpt records the two responses:
...
\noindent The phrase ``verified the semantic alignment'' is the model's own description; a four-character comparison does not establish semantic alignment or full-hash identity. The operational lesson is that the agent could invoke a precise comparison and make its response follow the reported branch result. This gives the workflow an explicit checkpoint instead of leaving definition agreement wholly implicit in conversation. The response excerpt does not prove that the tool caused the decision, that a downstream execution boundary was enforced, or that agents generally obey the tool outside this instructed scenario.
...
\begin{itemize}
    \item \textbf{Condition A (Natural Language)}: Agents coordinated without Sema tools or pattern references. The available configuration includes ordinary-language role, quality, and optimistic act-now instructions; the archived traces contain the corresponding intent-declaration syntax. It is a natural-language control, not an absence of coordination guidance.
...
The narrower observed turn-count ranges in Conditions B and C motivate the \emph{Stability Hypothesis}: a shared, structured vocabulary may reduce coordination variability by giving agents common procedures and stopping criteria. These runs show the descriptive pattern; they do not isolate that proposed mechanism from the accompanying prompts, workflow requirements, and tooling.

\begin{description}
    \item[The Tail Risk of Natural Language (Condition~A)] The baseline showed the widest observed range, completing in as few as 4 turns but extending to 51 turns in a coordination loop that lasted over 13 minutes. The long run illustrates the practical cost of ethical debate and circular reasoning failing to converge. It motivates a tail-risk concern alongside research on consensus difficulty in natural-language multi-agent systems~\cite{berdoz2026agree}; five observations do not establish a fat-tailed distribution or show that natural-language instructions lack useful guardrails.

    \item[The ``Sema Tax'' Hypothesis (Condition~B)] Condition~B had the narrowest turn-count range (10--19 turns; sample standard deviation $s=4.0$) and was slower on average than C (230 versus 203 seconds). A possible explanation is the coordination cost of looking up definitions and checking invariants---the ``Sema Tax''---consistent with costs discussed in prior work~\cite{cemri2025mast, chocron2020vocabulary}. The runs did not separately time those activities, so the overhead remains a hypothesis rather than a measured attribution.

    \item[The Protocol Multiplier Hypothesis (Condition~C)] Condition~C had the fastest mean completion time. Its experiment-time \texttt{OptimisticSolver} (\texttt{ae61}) and \texttt{AtomicBid} (\texttt{4b82}) definitions bundle intent and execution, suggesting a way to offset coordination overhead by avoiding separate permission exchanges. OptimisticSolver's Tier~2 design (Section~\ref{sec:contract_levels}) assumes alignment-seeking agents and deliberately trades some verification overhead for lower latency. This is the proposed ``Protocol Multiplier,'' not an isolated causal estimate: C had a wider turn-count range than B, but neither Sema condition reproduced A's longest loop in these five runs.
\end{description}

The traces also show the vocabulary being used to organize mechanism-oriented design and critique. In the first archived Condition~C run, the team moved from static bytecode monitoring toward \emph{Differential Trace Simulation}, then examined simulation-aware evasion and funding-lineage blind spots. This is a concrete example of agents proposing a causal mechanism, identifying how it can fail, and refining the proposal's stated constraints rather than merely citing pattern names.\footnote{See \path{experiments/sema_design_challenge/traces/run_20251229_162004/C_Optimistic/swarm.jsonl}, especially the revision request at line~113 and later critique at line~146. These are illustrative design claims from the agents, not externally validated trading mechanisms.} The \texttt{MechanisticDesignProposal} (\texttt{ad31}) structure was prescribed, so adoption is expected; its value as a scaffold for explicit causal attack models is a promising interpretation to test. Establishing improved design quality over the natural-language condition would require matched output requirements and an independent assessment that was not performed here.

%==============================================================================
\section{Related Work}
Sema combines several established lines of work. The comparisons below distinguish content identity, procedural vocabulary, interoperability and alignment, and runtime trust. This keeps the contribution specific: verified behavioral definitions used as terms in the language agents exchange, with explicit composition and an enforceable comparison boundary.
...
\item[Content Identity Across Domains] InChIKey hashes a molecular InChI representation into a compact chemical identifier~\cite{inchifaq}; Git and IPFS~\cite{benet2014ipfs} use content addresses for software and data. UniParc illustrates a related identity goal through a different mechanism: each unique protein sequence receives a stable archive identifier (UPI), rather than making that identifier a content hash~\cite{uniparcidentity}. These examples show why the representation and identity rule must be stated for each domain; they need not use the same encoding to motivate stable reference.
	...
	Ethereum ABI function selectors~\cite{ethereum2015abi} dispatch calls using a 4-byte Keccak-256 hash of a type signature; Ricardian Contracts~\cite{grigg1996ricardian} identify a human-readable contract by its hash; and Dhall~\cite{gonzalez2017dhall} hashes normalized expressions rather than source bytes. These precedents respectively connect identifiers to dispatch, contractual text, and canonical form. Sema brings those identity-oriented ideas to an agent vocabulary of reusable behavioral specifications.

\item[Code, Cognition, and Economy] Pattern Cards place reasoning procedures such as \sema{Reason}{81ec} and economic acts such as \sema{Bid}{79cf} in the same graph. The intuition is that an agent can inspect its commitments with the same compositional grammar it uses for its reasoning. IPFS~\cite{benet2014ipfs} supplies a content-addressing precedent, while Unison~\cite{chiusano2024unison} identifies functions through hashes of de-named abstract syntax trees. Pattern Cards apply a related definition-DAG idea to behavioral specifications, adding typed interfaces and handshake comparison. The name/identity separation has a precise limit here: renaming an object's own unhashed handle preserves its digest, but handles retained inside hashed references can affect dependent identities.
\end{description}
...
The proposed connection is a feedback loop: runtime use produces explicit pattern references and candidate refinements; a metacognitive training corpus can cite those artifacts; and training or evaluation can inform which formulations prove useful in later coordination. Shared content identities make the objects of that loop inspectable across stages. The training construction and the loop's effectiveness remain open work, while the runtime vocabulary, resolution, and minting mechanisms are implemented.
\end{description}
...
\paragraph{Demonstration Scale.} The multi-agent demonstration (Section~\ref{sec:vigilantflow}) used $N=5$ runs per condition with three agents in one task domain and model family. It motivates hypotheses rather than establishing general effects. A larger evaluation should vary architectures and domains, match prompts and non-hashed vocabulary content, and define outcome scoring in advance so it can distinguish vocabulary, tooling, and protocol effects.
...
\paragraph{Embedding Granularity.} The similarity screen uses \texttt{all-MiniLM-L6-v2} to embed each pattern's handle, gloss, and mechanism into 384 dimensions. Low similarity under that representation and exact mechanism-text uniqueness answer narrower questions than functional distinctness. Larger or domain-specific encoders may reveal different clusters; the \semaEmbHighPairs{} pairs at or above 0.70 remain candidates for documented comparison of meaningful variants and possible synonyms.

\paragraph{Definition Quality.} When agents verify the same full Pattern Card identifier, they establish canonical-definition identity, not that the definition is correct. A perfectly hashed pattern with weak invariants, incomplete failure modes, or an unsound mechanism can propagate bad coordination as reliably as good coordination. The adversarial contract review described here can expose specification errors, but it is not an automatic behavioral guarantee; the proposed \sema{Judge}{704c} scoring remains unimplemented. Definition quality depends on authorship, review, and eventual empirical testing. The protocol provides no defence against a well-hashed bad idea.

\paragraph{The Synonymy Limit.} Full-reference verification establishes identity, not canonical uniqueness. Two Pattern Cards that describe behaviorally equivalent mechanisms in different words---different invariant phrasings or different decompositions into primitives, even after field-order differences are canonicalized away---will produce different hashes and therefore count as different patterns. It does not guarantee that holders of behaviorally equivalent meanings hold the same hash. Detecting and merging such synonyms is a curation task supported by the Taxonomist workflow and the embedding-similarity audits described in Section~\ref{sec:standard_library}, not by the protocol itself. Refinement reduces synonymy in the bootstrap library, but content addressing remains agnostic to it: a community fork producing patterns equivalent to the canonical library would coexist with rather than collide with that library, and merging the two requires explicit alignment work outside the cryptographic layer.
