...
Sema inverts the separation between infrastructure and communication. By expressing a behavioral specification---invariants, preconditions, failure modes, typed dependencies---in a canonical form and hashing it, the resulting identifier becomes a token that agents embed directly in the natural language they already think in. An agent writes: ``I \sema{Delegate}{ec05} this \sema{Task}{a850} to you, expecting a \sema{Solution}{6bd0} that satisfies this \sema{AcceptSpec}{a724}.'' Each anchored term is simultaneously a word and a cryptographic proof. Because any change to a hashed semantic field produces a different hash---while reserialization, renaming, and metadata edits provably do not---any divergence in meaning produces a different identifier, revealing misalignment before it causes system failures (Figure~\ref{fig:pipeline}).\footnote{Large context windows ($>1$M tokens) do not subsume this mechanism: long contexts suffer from ``Lost in the Middle'' attention degradation, and two agents both holding a full definition in context still have no way to prove they hold the \emph{same} one. Sema hashes act as attention anchors and as cross-agent equality proofs that a shared window cannot provide.} This architecture represents content-addressing dissolved into language:
...
The same substrate functions as a collective memory with an unusual property: it survives compression. An agent's prose memory degrades at every summarization boundary---the words survive the summary, but their meanings quietly shift. A memory written with Sema references is pinned: the reference resolves to the same verified definition after any number of compaction cycles, so what accumulates across sessions and across agents is meaning rather than paraphrase.

Sema's claim on reasoning is specific about its mechanism. A reference contributes nothing when the model already holds a concept precisely: re-supplying textbook definitions costs tokens and buys no accuracy. The contribution runs through three channels instead. Verified premises---a conclusion built on checked references inherits their status, and a mismatch halts work before effort is spent on a false shared assumption. Checkable invariants available mid-task---a card's contract functions as a checklist an agent can test its own work against, forcing into consideration exactly the moves that fluent reasoning skips. And the naming of moves practitioners use but rarely write down, which training corpora underrepresent in proportion to their tacitness.
...
"dependencies": {
  "composes_with": {
    "sim": "sema:Simulation#mh:SHA-256:1476bfa1b0f3290a98ba6f83a3d44865d7fd148af9a1343508e0f066b5ab30d9",
    "vote": "sema:Vote#mh:SHA-256:d3cbfb3d419b84397dff409d2f83271e9bcb1b6c16b79b0b3629cbe72bd7f3a8"
  },
  "accepts": {
  ...
  "yields": {
    ...
    \textbf{Example:} \texttt{"\_meta": \{ "tier": 1, "related": ["Mutex\#d9da"], "supersedes": ["sema:OldHandle\#mh:SHA-256:..."] \}}. The unhashed \texttt{supersedes} list records version history without making the route by which a definition was authored part of its identity.
\end{description}
...
The Sema Compiler resolves these high-level intents into executable low-level handles. When an agent executes a polymorphic intent such as \texttt{Deep(Discover)}, the compiler queries the vocabulary graph for a pattern declaring that signature (e.g., \sema{DeepResearch}{52e2}) and resolves the dependency chain. This compilation step decouples intent from implementation, allowing the system to upgrade the underlying algorithms, such as swapping a heuristic check for a rigorous crypto-audit, without altering the agent's cognitive script.
...
\texttt{Deep(Heuristic)} & \sema{HeuristicSnap}{cecf} & \sema{MentalSim}{9ddd} \\
\texttt{Deep(Trace)} & \sema{Trace}{b72a} & \sema{GenealogicalTrace}{f30f} \\
\texttt{Deep(Discover)} & \sema{Discover}{f6e0} & \sema{DeepResearch}{52e2} \\
\texttt{Deep(Nature)} & \sema{Nature}{9ac1} & \sema{LivedProof}{85c2} \\ \bottomrule
\end{tabular}
\end{table}
...
(e.g., \sema{Lock}{95c2} $\rightarrow$ \sema{Mutex}{d9da}, or
\sema{Task}{a850} $\rightarrow$ \sema{BoundedTask}{fa24}). The test prevents
...
The vocabulary is not a flat list; it is structured into four fundamental layers mimicking a civilization stack. At the base lies \textbf{Physics}, substrate primitives that obtain regardless of any author: thermodynamics of coordination that cannot be ``wished away'' (e.g., \sema{Mutex}{d9da}, \sema{Entropy}{d5f7}, and recently-added substrate concepts \sema{Gradient}{ecfc}, \sema{Equilibrium}{4f03}, \sema{Conservation}{0b32}, \sema{Distance}{dfdf}, \sema{PhaseTransition}{b775}, \sema{Attractor}{0036}, \sema{MutualInformation}{e714}, and \sema{Measurement}{511d}). Above this rests \textbf{Mind}, mechanisms that structurally require cognition (e.g., \sema{BayesUpdate}{879a}), where judgment must be exercised and a single isolated agent can execute the mechanism. Emerging from these is \textbf{Society}, mechanisms that structurally require $\geq 2$ independent parties with potentially divergent state (e.g., \sema{Consensus}{ec30}). Enclosing all three is \textbf{Infrastructure} (Figure~\ref{fig:stack}): authored structures and operations that do not require cognition to execute---data types, composite topologies, and mechanical operations (e.g., \sema{ComputeBudget}{4091}).
...
The bootstrap library chose to tighten the rule through refinement (Section~\ref{sec:refinement}): hard dependencies (\texttt{accepts}, \texttt{composes\_with}) must now flow from higher layers to lower (Society $\rightarrow$ Mind $\rightarrow$ Infrastructure $\rightarrow$ Physics), enforced as a \texttt{sema apply} gate, while soft citations (\texttt{references}) and outputs (\texttt{yields}) are exempt and allow semantically necessary cross-layer links (for example, \sema{Belief}{675c} in Infrastructure soft-references \sema{Agent}{2176} in Mind because beliefs are conceptually held by agents, and the reference is acyclic). The current vocabulary contains \semaPatternCount~patterns organized into \semaCategoryCount~categories across these four layers (Table~\ref{tab:categories}; complete taxonomy in Appendix~\ref{app:taxonomy}), with zero patterns violating the layer-direction rule at this version.
...
The architecture is anchored by the \sema{PolymorphicSolver}{1362}, which implements the five-surface Solver Contract across any substrate (LLM, human, hybrid, tool-using agent, or nested composition) and acts as the default implementer within the \sema{UniversalSolverTree}{a6cb}. The abstract contract itself is \sema{Solver}{} --- an interface any \sema{Agent}{} can take on for the duration of a Task. Each Solver node presents a uniform interface but dynamically decides whether to execute the work directly or to decompose via \sema{ConceptualDecomposition}{cdf8} (distinct from generic \sema{Decompose}{db21} by its contract-bound requirement: each sub-concept exposes the Solver interface and is therefore delegatable) and delegate to child Solvers. The five surfaces: Manifest (\sema{Card}{ed28}), Execute (\sema{PolymorphicSolver}{1362} itself --- the universal atom), Consult (\sema{SocraticLoop}{e966}), Verify (\sema{Validate}{bd94}), and Feedback (\sema{PerformanceSignal}{0f41}). Inputs are encapsulated in a \sema{Task}{a850} with full constraint inheritance, while outputs are returned as a \sema{Solution}{6bd0} with full provenance. The \sema{ComputeBudget}{4091} acts as a pre-execution gate, and \sema{MarginalValueRule}{48c4} governs whether further decomposition is worth the cost. The claim that reasoning extends beyond the capacity of any individual model through typed composition is developed in~\cite{westerberg2026crp}.
...
\begin{description}
    ...
    \item[Tier 2 (Honesty-Dependent)] includes patterns with clear mechanisms that assume alignment-seeking agents. These are safe for cooperative contexts, but adversarial contexts require Tier~1 alternatives due to collusion risks (e.g., \sema{SteelmanCheck}{7611}).
...
The loop would proceed in four phases. First, in \textit{Hypothesize (Generativity)}, an agent detects friction or explores latent space (via \sema{ConceptBlend}{5d26}) to generate a candidate mechanism. Second, during \textit{Judge (Evaluative Merit)}, the candidate would be passed to the \sema{Judge}{74de} primitive for a scalar assessment of structural merit. Third, the \textit{Harden (Adversarial Evolution)} phase would subject the pattern to \sema{AdversarialSteel}{aaf9}, where a ``Devil's Advocate'' attempts to exploit its invariants. Finally, in \textit{Mint (Crystallization)}, the survivor is hashed, becoming an immutable pattern available for immediate citation and reuse. The infrastructure for the final step---validation, hashing, and atomic minting via the \texttt{sema\_mint} tool---is fully implemented; the preceding three steps require either human judgment or LLM-based evaluation that remains future work.
...
To enable the Genesis Loop, we introduce a new Ring~0 primitive: \sema{Judge}{74de}.

While \sema{Check}{6890} queries \emph{Validity} (True/False), \sema{Judge}{74de} is specified to query \emph{Merit} (0.0 to 1.0). Unlike probabilistic claim analysis systems that rely on model confidence~\cite{wadden2020fact, metropolitansky2025claimify}, the proposed Judge would evaluate structural properties of the pattern (connectivity, orthogonality) rather than its truth value. It is intended to operationalize the ``metacognitive training'' that distinguishes deep insight from sophisticated rhetoric.
\begin{equation}
...
The implementation has three boundaries readers should distinguish before §6's evaluation. \emph{Implemented and enforced}: canonicalization, hashing, schema validation, atomic mutation, dependency graph constraints, and the MCP handshake. \emph{Implemented but optional}: hybrid search, embedding navigation, dependency expansion, browser tooling, pull-mode synchronization, and the Taxonomist refinement role---these run as agent or operator workflows but are not protocol-level requirements. \emph{Specified but not enforced at runtime}: the \sema{Judge}{74de} primitive's scoring, the Genesis Loop, and the Experience Matrix---these are described in §\ref{sec:living_taxonomy} as the targets the implemented infrastructure was designed to support, with their current status catalogued in §\ref{sec:limitations}.
...
\begin{description}
    ...
    \item[Strawman Waltz Collusion (Mind)] In \sema{SteelmanCheck}{7611}, both proposer and critic are the same agent. A misaligned agent can generate deliberately weak counter-arguments (``strawmen'') and then easily defeat them, claiming the plan ``survived'' critique.
    \par\textbf{Defense:} This pattern was downgraded to Tier~2. We added precondition: ``Agent must be alignment-seeking; use \sema{AdversarialSteel}{aaf9} for adversarial contexts.'' The failure mode now explicitly documents collusion risk.
    ...
    \par\textbf{Defense:} We added a \emph{Holographic Inheritance Invariant} to Task: ``Constraints propagate through ALL boundaries (tool calls, sub-agents, delegations). No opacity escape.'' The pattern \sema{ToolInvoke}{aafc} now explicitly declares: ``Tool Execution Context permissions MUST be $\leq$ Task Constraints.''

    \item[The Semantic Inversion Attack (Topology)] An adversary (or a careless architect) creates a valid DAG that is semantically inverted, such as making \sema{Deduction}{b9a0} depend on \sema{Vote}{d3cb}. While computationally valid, this makes truth fragile to political change.
    ...
    \begin{enumerate}
        \item \emph{Dependency Inversion}: We resolved the dependency cycle between \sema{Agent}{2176} and \sema{Act}{2dfe} by splitting the concept. We created \sema{Actor}{86fc} (Infrastructure) as the ``capability container'' that executes acts, while \sema{Agent}{2176} (Mind) remains the reasoning entity. Infrastructure patterns that need a capability holder reference Actor; patterns that need a reasoner still reference Agent.
...
\begin{quote}
\small\ttfamily
[21:06:41] verifier: ``I have verified the semantic alignment for the Abduction\#b62e pattern. The handshake returned PROCEED, so I agree to use this pattern.'' \\
{[21:06:44]} attacker: ``The handshake for Abduction\#b62e with hash deadbeef returned HALT due to semantic drift. Therefore, I must reject the coordination.''
\end{quote}
...
\begin{itemize}
    ...
    \item \textbf{Condition C (Vocabulary + Protocol)}: Agents used \sema{OptimisticSolver}{f664} with the \sema{AtomicBid}{820c} protocol.
\end{itemize}
...
\begin{description}
    ...
    \item[The Protocol Multiplier (Condition~C)] The \sema{OptimisticSolver}{f664} achieved the fastest average completion time. By using \sema{AtomicBid}{820c} to bundle intent and execution, it effectively ``refunded'' the cognitive cost of using the vocabulary. We hypothesize that this speed advantage arises from OptimisticSolver's Tier~2 design (Section~\ref{sec:contract_levels}): by assuming alignment-seeking agents, it trades verification overhead for reduced latency. While slightly less stable than B, it consistently avoided the catastrophic spirals of A.
\end{description}

Qualitative analysis verified vocabulary adoption by extracting pattern references. Condition~C agents autonomously employed \sema{MechanisticDesignProposal}{2f7a} to structure their outputs, often identifying complex attack vectors (e.g., ``Differential Trace Simulation'' vs.\ ``Static Analysis'') that the Baseline agents missed. Whereas the Baseline produced generic compliance-oriented solutions, the Sema agents more often produced mechanism-oriented designs with explicit causal attack models.
...
\begin{description}
\item[C1: Hash-as-word inversion.] Sema moves content-addressed identity into the language agents already use. A Sema word such as \sema{Delegate}{ec05} is both a readable handle and a verifiable pointer to a canonical behavioral contract.
