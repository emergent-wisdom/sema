...
\begin{tikzpicture}[
    ...
    \node[box, fill=semagreen!10, draw=semagreen, right=of hash] (id) {\textbf{Identifier}\\ \texttt{\scriptsize StateLock\#5602}};
...
"dependencies": {
  "composes_with": {
    "sim": "sema:Simulation#mh:SHA-256:5e80a6ebef4ecbdf66157f03cd64c7b5d7b5663c11c6d50d3166ba95c9087530",
    "vote": "sema:Vote#mh:SHA-256:37f80069661e4c80fc47d051b967ac6fa103d950da07ab627cebc037a70c319f"
  },
  "accepts": {
    "proposal": "sema:Proposal#mh:SHA-256:ab244c2a0c2715df0b7c48b200d94c62c207ff9f7510f7df4d45b6d5a698eae0"
  },
  "yields": {
    ...
    \textbf{Example:} \texttt{"derived\_from": "Trace\#2836"}. This enables the system to construct the ``Tree of Thought.''
    ...
    \textbf{Example:} \texttt{"\_meta": \{ "tier": 1, "related": ["Mutex\#4f92"] \}}.
\end{description}
...
Let $|r|$ be reference length (handle + 4-char hash $\approx$ 15 characters) and $|d|$ be definition length. Compression ratio is $|d|/|r|$. Expansion against the referenced canonical object is deterministic: given access to $d$, $\text{expand}(r) = d$ is a pure function returning identical bytes on every invocation.
\end{proposition}
...
The Sema Compiler resolves these high-level intents into executable low-level handles. When an agent executes a polymorphic intent such as \texttt{Deep(Discover)}, the compiler queries the vocabulary graph for a pattern declaring that signature (e.g., \sema{DeepResearch}{c94a}) and resolves the dependency chain. This compilation step decouples intent from implementation, allowing the system to upgrade the underlying algorithms, such as swapping a heuristic check for a rigorous crypto-audit, without altering the agent's cognitive script.
...
\texttt{Deep(Heuristic)} & \sema{HeuristicSnap}{abd5} & \sema{MentalSim}{2ec6} \\
\texttt{Deep(Trace)} & \sema{Trace}{2836} & \sema{GenealogicalTrace}{18d2} \\
\texttt{Deep(Discover)} & \sema{Discover}{e889} & \sema{DeepResearch}{c94a} \\
\texttt{Deep(Nature)} & \sema{Nature}{6c1a} & \sema{LivedProof}{3da4} \\ \bottomrule
\end{tabular}
\end{table}
...
semantics, as with \sema{StateLock}{5602}---or if specifying it produces
...
context-specific features become descendant territory via \texttt{derived\_from}
(e.g., \sema{Lock}{051c} $\rightarrow$ \sema{Mutex}{4f92}, or
\sema{Task}{b290} $\rightarrow$ \sema{BoundedTask}{1063}). The test prevents
...
The vocabulary is not a flat list; it is structured into four fundamental layers mimicking a civilization stack. At the base lies \textbf{Physics}, substrate primitives that obtain regardless of any author: thermodynamics of coordination that cannot be ``wished away'' (e.g., \sema{Mutex}{4f92}, \sema{Entropy}{a265}, and recently-added substrate concepts \sema{Gradient}{480b}, \sema{Equilibrium}{f7c5}, \sema{Conservation}{ba2e}, \sema{Distance}{1376}, \sema{PhaseTransition}{edf8}, \sema{Attractor}{487f}, \sema{MutualInformation}{da31}, and \sema{Measurement}{5da6}). Above this rests \textbf{Mind}, mechanisms that structurally require cognition (e.g., \sema{BayesUpdate}{bbf6}), where judgment must be exercised and a single isolated agent can execute the mechanism. Emerging from these is \textbf{Society}, mechanisms that structurally require $\geq 2$ independent parties with potentially divergent state (e.g., \sema{Consensus}{376f}). Enclosing all three is \textbf{Infrastructure} (Figure~\ref{fig:stack}): authored structures and operations that do not require cognition to execute---data types, composite topologies, and mechanical operations (e.g., \sema{ComputeBudget}{ff07}).
...
The bootstrap library chose to tighten the rule through refinement (Section~\ref{sec:refinement}): hard dependencies (\texttt{accepts}, \texttt{composes\_with}) must now flow from higher layers to lower (Society $\rightarrow$ Mind $\rightarrow$ Infrastructure $\rightarrow$ Physics), enforced as a \texttt{sema apply} gate, while soft citations (\texttt{references}) and outputs (\texttt{yields}) are exempt and allow semantically necessary cross-layer links (for example, \sema{Belief}{1f44} in Infrastructure soft-references \sema{Agent}{2072} in Mind because beliefs are conceptually held by agents, and the reference is acyclic). The current vocabulary contains \semaPatternCount~patterns organized into \semaCategoryCount~categories across these four layers (Table~\ref{tab:categories}; complete taxonomy in Appendix~\ref{app:taxonomy}), with zero patterns violating the layer-direction rule at this version.
...
The vocabulary is not a flat list of tools but a compositional grammar that enables agents to construct novel intents from atomic units. Ring~0 \emph{Primitives} act as the fundamental building blocks of agent cognition. Five foundational verbs map to dimensions of autonomous inquiry: \sema{Discover}{e889} (Breadth) acts as the Scout, querying the horizon to find new nodes, agents, or paths; \sema{Deep}{89f0} (Depth) serves as the Scientist, escalating from heuristic proxies to rigorous implementations; \sema{Trace}{2836} (Time) functions as the Historian, reconstructing causal chains and ensuring provenance; \sema{Check}{5f38} (Validity) operates as the Critic, filtering inputs against invariants; and \sema{Stigmergy}{53d4} (Space) works as the Builder, writing persistent signals for indirect coordination. Alongside these verbs, noun primitives such as \sema{Belief}{1f44}, \sema{Agent}{2072}, and \sema{Constraint}{87fe} provide the objects of agency.

These primitives enable a higher-order grammar where complex behaviors compose from atomic units: \sema{Check}{5f38}(Condition) evaluates a target \sema{Condition}{cbd5} and enforces a fail-closed halt if False; \sema{Trace}{2836}(Target) wraps a process to generate an immutable lineage log; and \sema{Stigmergy}{53d4}(Signal) marks the shared environment with a signal for others to discover.

Ring~2 \emph{Macros} are executable patterns composed of primitives via semantic import. (Ring 0--2 describes compositional complexity; Tier 1--3, Section~\ref{sec:contract_levels}, describes safety rigor. Both are stored in the unhashed metadata overlay.) For example, \sema{AgentDiscover}{42ca} imports \texttt{\{\{Discover\#e889\}\}} acting on \texttt{\{\{Agent\#2072\}\}}, while \sema{TraceBelief}{22cc} imports \texttt{\{\{Trace\#2836\}\}} acting on \texttt{\{\{Belief\#1f44\}\}}. This grammar allows agents to express intent via queries like ``I need to \texttt{Deep(Trace)} this belief,'' which the runtime resolves to concrete executable patterns.

This distinction reveals a bicameral architecture. Primitives serve as the query interface, allowing agents to identify logical gaps in the vocabulary. Macros like \sema{IdentityHandshake}{fb21} serve as the social interface, enabling efficient coordination through compression. Instead of negotiating four separate primitives in four round-trips, agents verify a single Macro hash in one. Primitives are for querying; Macros are for talking.

The verb/noun distinction introduced in the Type System (Section~\ref{sec:type_system}) also addresses the ``schema drift'' problem where implicit schemas lead to subtle incompatibilities. Sema formalizes the ``shape of work'' into content-addressed Data Patterns. \sema{Task}{b290} represents a recursive definition of intent that inherits constraints from its parent and explicitly defines its \texttt{AcceptSpec}. \sema{Solution}{9893} acts as a container for work product, encapsulating provenance and a component tree for supply-chain auditing. \sema{UniqueHandle}{e9b3} serves as a rivalrous resource pointer obeying linear logic to prevent double-spending of physical assets; if Agent~A transfers the handle to Agent~B, Agent~A loses access immediately. To ensure structural compatibility, Noun definitions may include a validation schema in the \texttt{data\_schema} field (e.g., \sema{Task}{b290}, \sema{Bid}{464a}), embedding constraints directly into the identity unlike external standards such as SHACL~\cite{knublauch2017shacl}. This allows agents to construct rigorous semantic sentences:

\begin{quote}
\textit{``I \sema{Delegate}{ba86} this \sema{Task}{b290} to you, expecting a \sema{Solution}{9893} that satisfies this \sema{AcceptSpec}{b77c}.''}
\end{quote}
...
The architecture is anchored by the \sema{PolymorphicSolver}{d104}, which implements the five-surface Solver Contract across any substrate (LLM, human, hybrid, tool-using agent, or nested composition) and acts as the default implementer within the \sema{UniversalSolverTree}{c523}. The abstract contract itself is \sema{Solver}{} --- an interface any \sema{Agent}{} can take on for the duration of a Task. Each Solver node presents a uniform interface but dynamically decides whether to execute the work directly or to decompose via \sema{ConceptualDecomposition}{75bc} (distinct from generic \sema{Decompose}{dcf9} by its contract-bound requirement: each sub-concept exposes the Solver interface and is therefore delegatable) and delegate to child Solvers. The five surfaces: Manifest (\sema{Card}{6848}), Execute (\sema{PolymorphicSolver}{d104} itself --- the universal atom), Consult (\sema{SocraticLoop}{e9e5}), Verify (\sema{Validate}{7b5b}), and Feedback (\sema{PerformanceSignal}{b9d6}). Inputs are encapsulated in a \sema{Task}{b290} with full constraint inheritance, while outputs are returned as a \sema{Solution}{9893} with full provenance. The \sema{ComputeBudget}{ff07} acts as a pre-execution gate, and \sema{MarginalValueRule}{311b} governs whether further decomposition is worth the cost. The claim that reasoning extends beyond the capacity of any individual model through typed composition is developed in~\cite{westerberg2026crp}.
...
\begin{description}
    ...
    \item[Tier 2 (Honesty-Dependent)] includes patterns with clear mechanisms that assume alignment-seeking agents. These are safe for cooperative contexts, but adversarial contexts require Tier~1 alternatives due to collusion risks (e.g., \sema{SteelmanCheck}{2d13}).
...
To enable the Genesis Loop, we introduce a new Ring~0 primitive: \sema{Judge}{efe0}.

While \sema{Check}{5f38} queries \emph{Validity} (True/False), \sema{Judge}{efe0} is specified to query \emph{Merit} (0.0 to 1.0). Unlike probabilistic claim analysis systems that rely on model confidence~\cite{wadden2020fact, metropolitansky2025claimify}, the proposed Judge would evaluate structural properties of the pattern (connectivity, orthogonality) rather than its truth value. It is intended to operationalize the ``metacognitive training'' that distinguishes deep insight from sophisticated rhetoric.
\begin{equation}
...
{
  "handle": "BoundedTask",
  "derived_from": "sema:Task#mh:SHA-256:b290c7f79fd936d3a3035c5b35658f94b85b6e1b749b1916576a1cf587d8e8c5",  // The Phylogeny
  "mechanism": "A specialized Task enforcing budget...",
  "dependencies": { ... }  // The Wiring & Interface
}
\end{lstlisting}

This lineage tracking allows the AI to query the \emph{Meta-Patterns}: ``Show me the evolutionary path from generic \sema{Task}{b290} to specialized \sema{BoundedTask}{1063}.'' The system can identify which lineages are stagnant and which are undergoing rapid speciation (innovation).
...
The implementation has three boundaries readers should distinguish before §6's evaluation. \emph{Implemented and enforced}: canonicalization, hashing, schema validation, atomic mutation, dependency graph constraints, and the MCP handshake. \emph{Implemented but optional}: hybrid search, embedding navigation, dependency expansion, browser tooling, pull-mode synchronization, and the Taxonomist refinement role---these run as agent or operator workflows but are not protocol-level requirements. \emph{Specified but not enforced at runtime}: the \sema{Judge}{efe0} primitive's scoring, the Genesis Loop, and the Experience Matrix---these are described in §\ref{sec:living_taxonomy} as the targets the implemented infrastructure was designed to support, with their current status catalogued in §\ref{sec:limitations}.
...
\begin{description}
    \item[Blinking Light Livelock (Physics)] A malicious agent spams lock requests on \sema{StateLock}{5602}, acquiring and releasing locks at maximum speed while technically respecting timeouts. This denies access to legitimate agents without violating any stated invariant.
    \par\textbf{Defense:} The pattern now composes with \sema{Backoff}{c6d1} and \sema{Cooldown}{6eb2} via explicit dependencies, enforcing exponential backoff and a mandatory cooldown period between lock acquisitions. The defense is structural (dependency composition) rather than declarative (an invariant string).
    ...
    \par\textbf{Defense:} We added a \emph{Fail-Fast Invariant}: ``On hash mismatch, DO NOT RETRY tuning. Halt immediately and escalate to \sema{OntologyHandshake}{8443} or human review.'' A new parameter \texttt{max\_retries: Integer [0, 1]} (default 0) enforces this at the contract level.

    \item[Strawman Waltz Collusion (Mind)] In \sema{SteelmanCheck}{2d13}, both proposer and critic are the same agent. A misaligned agent can generate deliberately weak counter-arguments (``strawmen'') and then easily defeat them, claiming the plan ``survived'' critique.
    \par\textbf{Defense:} This pattern was downgraded to Tier~2. We added precondition: ``Agent must be alignment-seeking; use \sema{AdversarialSteel}{ded9} for adversarial contexts.'' The failure mode now explicitly documents collusion risk.

    \item[Trojan Horse Sandbox Escape (Infrastructure)] An agent wraps a prohibited action (e.g., network access) inside an opaque \texttt{ToolInvoke} call, hoping the outer \sema{Task}{b290} constraints don't propagate into the tool's execution context.
    \par\textbf{Defense:} We added a \emph{Holographic Inheritance Invariant} to Task: ``Constraints propagate through ALL boundaries (tool calls, sub-agents, delegations). No opacity escape.'' The pattern \sema{ToolInvoke}{bd2b} now explicitly declares: ``Tool Execution Context permissions MUST be $\leq$ Task Constraints.''

    \item[The Semantic Inversion Attack (Topology)] An adversary (or a careless architect) creates a valid DAG that is semantically inverted, such as making \sema{Deduction}{9c88} depend on \sema{Vote}{37f8}. While computationally valid, this makes truth fragile to political change.
    ...
    \begin{enumerate}
        \item \emph{Dependency Inversion}: We resolved the dependency cycle between \sema{Agent}{2072} and \sema{Act}{dc2d} by splitting the concept. We created \sema{Actor}{57f6} (Infrastructure) as the ``capability container'' that executes acts, while \sema{Agent}{2072} (Mind) remains the reasoning entity. Infrastructure patterns that need a capability holder reference Actor; patterns that need a reasoner still reference Agent.
...
\begin{quote}
\small\ttfamily
[21:06:41] verifier: ``I have verified the semantic alignment for the Abduction\#68f6 pattern. The handshake returned PROCEED, so I agree to use this pattern.'' \\
{[21:06:44]} attacker: ``The handshake for Abduction\#68f6 with hash deadbeef returned HALT due to semantic drift. Therefore, I must reject the coordination.''
\end{quote}
...
\begin{itemize}
    ...
    \item \textbf{Condition C (Vocabulary + Protocol)}: Agents used \sema{OptimisticSolver}{8935} with the \sema{AtomicBid}{c429} protocol.
\end{itemize}
...
\paragraph{Specified but Not Built.} Several mechanisms described in the Living Taxonomy section (Section~\ref{sec:living_taxonomy}) are specifications rather than running code. The \sema{Judge}{efe0} primitive is formally defined as a Ring-0 evaluator but has no scoring implementation: quality assessment in the current system depends on manual review or external LLM evaluation. The Genesis Loop (Hypothesize $\rightarrow$ Judge $\rightarrow$ Harden $\rightarrow$ Mint) is a protocol for vocabulary self-evolution but is not automated; pattern creation remains human- or agent-initiated through \texttt{sema\_mint}. The Experience Matrix, which proposes content-addressed tensor transfer between agents (``Experiential Telepathy,'' Section~\ref{sec:experience_matrix}), requires serialization infrastructure not yet built. These represent the system's most speculative claims and require experimental validation before deployment.
