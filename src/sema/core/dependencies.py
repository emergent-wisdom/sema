...
def get_dependencies_handles(p: dict) -> set[str]:
    ...
    Includes `extends`. That field is a hashed reference to an exact parent
    definition, so the parent handle must be minted first (topological_sort) and a
    cycle through the specialization relation is a real cycle
    (build_dependency_adjacency). It is deliberately not part of the ordinary hash
    cascade: retargeting a child to a newer parent is an explicit semantic edit.

    Legacy `derived_from` is deliberately excluded. Before 0.4 it was an opaque
    hashed string with no graph semantics; compatibility must not reinterpret a
    historical card as a checked `IS_A` claim.
    """
    deps = set()
    ...
    parent = clean_handle(p.get("extends"))
    if parent:
        deps.add(parent)
