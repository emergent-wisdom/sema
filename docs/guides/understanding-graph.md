...
> hashes from the current sema vocabulary (`StateLock#f165`,
...
graph_batch({
  ...
  operations: [{
    ...
    mechanism: "Use sema://StateLock#f165 for session-level mutex.",
...
`StateLock#f165` to verify the definition is still the same one the original
architect used.
...
```
graph_semantic_search({ query: "StateLock#f165" })
```
