...
12. *Full-sweep annotation — `audits/2026-04-17/layer-assignment.md` + 16 additional proposed moves.* With the layer test locked, a complete classification pass across all 444 patterns produced `audits/2026-04-17/layer-assignment.md`: every handle, the proposed layer, and a one-line reason. The sweep triggered a third refinement of the layer principle, captured in the spec:

    **Physics is components (Nouns), not just substrate.** An earlier draft of `docs/core/philosophy.md §3.1` defined Physics narrowly as "obtains regardless of any author." That was too strict — it would evict legitimate foundational components (`Gate`, `Heartbeat`, `Throttle`, `Branch`, `Route`, `Cooldown`, `Hysteresis`) that are authored but act as building-block Nouns the rest of the library composes with. The correct framing is: Physics is the **bottom of the compose-with stack** — whether a component is substrate-given (`Lock`) or authored-but-foundational (`Gate`), it lives in Physics when its role is to be *a component other layers use* rather than *an assembly of other components*. The spec section was updated to capture this two-axis view (scope + shape): Physics Nouns vs. Infrastructure/Primitives Verbs, with the "is this a compose-with Noun or a built-from Verb" heuristic.

    **16 additional proposed moves** surfaced by the full pass (not yet applied — Round 7 review pending):

    **Physics → Infrastructure (6 Verbs)** — the six current Physics patterns that are *Verbs* (operations using substrate components) rather than *Nouns* (components themselves):
    - `Compensate` (the act of running logged inverses), `Compress` (the act of reducing), `EntropyPump` (the act of injecting entropy), `ReAttempt` (the act of retrying), `Sign` (the act of attaching signature), `StateAudit` (the act of verifying post-write)
    - The seven current Physics Nouns (`Branch`, `Gate`, `Route`, `Heartbeat`, `Cooldown`, `Throttle`, `Hysteresis`) **stay in Physics** as foundational components.

    **Physics → Society (1)**: `StateLock` — "two actors temporarily fuse state" is N=2 by mechanism.

    **Mind → Infrastructure (3)**: `Cache` (keyed lookup, no cognition); `Rank` (deterministic sort); `Decision` (committed-choice artifact — a Noun, not the cognitive Verb `Decide`).

    **Mind → Society (1)**: `Stigmergy` — mechanism structurally requires marker + reader on shared substrate.

    **Society → Infrastructure (2)**: `FrameSpec`, `ProtoPack` — typed data artifacts, not coordination acts.

    **Society → Mind (1)**: `ManifestPlanning` — architectural planning is cognitive; one mind transforms FrameSpec → ExecutionManifest.

    **Infrastructure → Mind (1)**: `Budget` — the *discipline* of respecting a resource ceiling is cognitive; borderline with the arithmetic of tracking.

    **Society → Infrastructure (TriGate)**: initially flagged Society on "governance seams" grounds, corrected — TriGate is an authored three-state gate primitive with N=1, belongs alongside Gate.

    **Post-hoc reasoning corrections** (retractions on inspection):
    - `Protocol` and `OathBind`: I initially proposed Infrastructure → Society on "between agents" / "actor + enforcer" framing. Retracted: Protocol is the *spec* (rules + formats — a Noun one agent can read), and OathBind is self-binding against code-enforced rulesets (N=1, Ulysses pact at protocol level). Both stay Infrastructure.
    - The seven Physics Nouns (`Branch`, `Gate`, `Route`, `Heartbeat`, `Cooldown`, `Throttle`, `Hysteresis`): initially proposed Physics → Infrastructure on "authored, not substrate" grounds. Retracted after the Physics-is-components refinement: they're foundational building-block Nouns that higher layers compose with. Stay Physics.

    These retractions exemplify the drift-to-Society and the strip-Physics-to-substrate traps. Mechanism context, not application surface or over-narrow substrate definition, decides the layer.

    **Projected distribution after applying the 16 remaining moves**:
    - Physics: 23 → **17** (−6 Verbs to Infrastructure, −1 StateLock to Society; seven Physics Nouns retained as components)
    - Infrastructure: 127 → **139** (+12 net)
    - Mind: 181 → **179** (net −2, as some Mind patterns are mechanical data)
    - Society: 113 → **111** (net −2)
